...
def grid_values(grid):
    ...
    """
    # make a new list for the grid in which '.' is substituted by '123456789'
    digits = '123456789'
    expanded_grid = []
    for c in grid: 
        if c == '.':
            expanded_grid.append(digits)
        else: 
            expanded_grid.append(c)
    # check the size is correct 
    assert len(expanded_grid) == 81
    return dict(zip( boxes, expanded_grid ))
...
def eliminate(values):
    ...
    # also eliminate the naked twins 
